...
export function recommendedPlanId(currentPlanId, kind) {
  ...
  // Pipelines: stepping up one tier doesn't always lift the cap (Free and
  // Student are both 6). Recommend the next tier that ACTUALLY raises it.
  if (kind === "pipeline") {
    const currentMax = getPlan(currentPlanId).maxPipelineSteps ?? 1;
    for (let i = idx + 1; i < PLAN_IDS.length; i++) {
      if ((getPlan(PLAN_IDS[i]).maxPipelineSteps ?? 1) > currentMax) {
        return PLAN_IDS[i];
      }
    }
    return null; // nothing higher lifts it (already effectively maxed)
  }
...
export function upgradeOffer(parsed, region = "global") {
  ...
  const titles = {
    ...
    pipeline: "Big multi-step job?",
  };
  const hooks = {
    conversions:
      current.quotaPeriod === "day"
        ? `You've used every conversion on ${current.name} today. ${target.name} gives you a lot more headroom — plus pay-as-you-go so you're never blocked again.`
        : `You've used every conversion on ${current.name} this month. ${target.name} gives you a lot more headroom — plus pay-as-you-go so you're never blocked again.`,
    ...
    pipeline: `${current.name} handles up to ${current.maxPipelineSteps} changes in a single request. ${target.name} takes you to ${target.maxPipelineSteps} — chain more edits in one go.`,
  };
  ...
  const perks = [
    ...
    target.maxPipelineSteps > 1
      ? `Chain up to ${target.maxPipelineSteps} edits in one request`
      : null,
