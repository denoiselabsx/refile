 ...
 *   - Quotas: Free 10/day (resets at UTC midnight, hard stop, no overage);
 *     Student 100/mo, Pro 750/mo, Power 3000/mo (all paid plans flow into
 *     metered overage at $0.02 each beyond their included monthly amount).
 *   - $0.02 per conversion over the included amount (paid tiers only),
 *     protected by a real-provider-cost floor (see computeOverage).
 *   - File-size caps: 100 MB / 250 MB / 500 MB / 2 GB.
 *   - Max pipeline steps: 6 / 6 / 12 / 12 (each step = 1 conversion, so a
 *     pipeline draws from the monthly quota; Free is self-limited by its
 *     15/mo hard stop).
 *
...
const PLAN_BASE = {
  free: {
    ...
    // Free resets DAILY at UTC midnight; paid plans bill monthly. This
    // matches what CloudConvert and similar tools do and turns the quota
    // from a one-time stress point into a daily reason to come back.
    quotaPeriod: "day",
    includedConversions: 10,
    overagePerConversion: null, // null = hard stop, no pay-as-you-go
    maxFileBytes: 100 * MB,
    maxFilesPerConversion: 1,
    // Max steps in one multi-step pipeline. Each step = 1 conversion, so a
    // Free pipeline is self-limited by the 15/mo hard stop. Pro/Power get a
    // generous-but-bounded cap (unbounded would risk action timeouts +
    // degrade LLM plan reliability — see runJob validatePlan).
    maxPipelineSteps: 6,
    maxPresets: 3,
    historyLimit: 30,
    support: "Community",
